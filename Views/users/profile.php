...
<div class="container mt-5">
    <div class="flex flex-row p-6 mt-5 bg-white border rounded-lg border-gray-200/80">
        <!-- Avaar Container -->
        <div class="relative">
            <!-- User Avatar -->
            <?php if (!empty($user->getAvatar())) : ?>
                <img class="object-cover w-40 h-40 rounded-md" src="<?= $user->getAvatar() ?>" alt="<?= $user->getFirstName() ?> <?= $user->getLastName() ?>?>">
            <?php else : ?>

                <div class="flex items-center justify-center w-40 h-40 font-bold text-white bg-purple-900 rounded-md" src="<?= $user->getAvatar() ?>" alt="<?= $user->getFirstName() ?> <?= $user->getLastName() ?>?>">
                    <p class="text-5xl"><?= $user->getFirstName()[0] ?></p>
                </div>
            <?php endif; ?>


            <!-- Online Status Dot -->
            <div class="absolute w-5 h-5 bg-green-400 border-4 border-white rounded-full -right-3 bottom-5 sm:top-2 sm:invisible md:visible" title="User is online"></div>
        </div>
        ...
        <div class="flex flex-col px-6">
            ...
            <div class="flex flex-row items-center h-8">
                <!-- Username -->
                <h2 class="text-lg font-semibold"><?= $user->getFirstName() ?> <?= $user->getLastName() ?></h2>

                <!-- User Verified -->
                <svg class="h-5 my-auto ml-2 fill-blue-400" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="24" height="24" viewBox="0 0 24 24">
            ...
            <div class="flex flex-row my-2 space-x-2">
                ...
                <div class="flex flex-row">
                    <svg class="w-4 h-4 mr-2 fill-gray-500/80" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="24" height="24" viewBox="0 0 24 24">
                ...
                <div class="flex flex-row">
                    ...
                    <div class="text-xs text-gray-400/80 hover:text-gray-400"><?= $user->getEmail() ?></div>
                </div>
            </div>

            <!-- Mini Cards -->
            <div class="flex flex-row items-center mt-2 space-x-5">
                <!-- Comments -->
                <a href="#" class="flex flex-col items-center justify-center w-40 h-20 transition-colors duration-100 ease-in-out border border-gray-200 border-dashed rounded-md hover:border-gray-400/80">
        ...
        <div class="flex flex-col items-end justify-start flex-grow w-100">
            <div class="flex flex-row space-x-3">
                <!-- Follow Button -->
                <button class="flex px-4 py-2 text-white transition-all duration-150 ease-in-out bg-blue-500 rounded-md hover:bg-blue-600">
